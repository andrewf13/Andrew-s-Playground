...
document.getElementById('total').textContent=String(window.PROJECTS.length).padStart(2,'0');
for(const b of document.querySelectorAll('[data-filter]')){const count=b.dataset.filter==='All'?window.PROJECTS.length:window.PROJECTS.filter(p=>p.category===b.dataset.filter).length;b.querySelector('span').textContent=count;}
...
for(const p of window.PROJECTS){
 ...
 const cover=p.id==='shield-dojo'?'<div class="preview-frame"><iframe src="assets/dojo-preview.html" title="Shield Dojo artwork preview" tabindex="-1" aria-hidden="true" loading="lazy"></iframe><div class="cover-caption"><span>BOUNCE EDITION v3</span><b>SHIELD<br>DOJO</b></div></div>':`<div class="type-cover"><div class="cover-top"><span>${escapeHTML(p.kind)}</span><span>${p.number} / ${String(window.PROJECTS.length).padStart(2,"0")}</span></div><strong>${escapeHTML(p.line1)}<br>${escapeHTML(p.line2)}</strong><div class="cover-bottom"><span>${escapeHTML(p.edition)}</span><b>${p.year}</b></div></div>`;
